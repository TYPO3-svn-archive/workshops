 ...
 *
 *
 * $Id: tx_workshops_languageMenu.php,v 0.0.1 2005/01/17 16:36:13 spadberg Exp $
 *
 * @author Dipl.-Ing. Stefan Padberg <post@webskriptorium.com>
 */

/**
 * language menu that keeps the links vars from tx_workshops
 *
 * @param	[type]		$content: ...
 * @return	void
 * @access public
...
function user_languageMenu($content) {
	...
	if ($queryString) {
		...
		$excludeList = 'id,L,tx_workshops[pointer]';
		...
		$itemsAddParams = $tmpParams?'&' . $tmpParams:'';
	}
	...
	if ($GLOBALS['TSFE']->page['l18n_cfg']==1){ // = "Hide default Translation" is activated
		...
		$flags[0] = ($tmpLang == 0?$pointer:'') . $GLOBALS['TSFE']->cObj->typolink($defaultflag['on'], array('parameter' => $GLOBALS['TSFE']->id . ' _top', 'additionalParams' => (!preg_match('/&L=[0-9]/', $workshopsAddParams)?$workshopsAddParams . '&L=0':$workshopsAddParams)));
	}
	...
	if (is_array($flagimages)) {
		foreach ($flagimages as $fk => $fv) {
			if ($langArr[$fk]) {
				$flags[$fk] = ($tmpLang == $fk?$pointer:'') . $GLOBALS['TSFE']->cObj->typolink($fv['on'], array('parameter' => $GLOBALS['TSFE']->id . ' _top', 'additionalParams' => (!preg_match('/&L=[0-9]/', $workshopsAddParams)?$workshopsAddParams . '&L=' . $fk:$workshopsAddParams)));
			} else {
				$flags[$fk] = $fv['off'];
			}
		}
