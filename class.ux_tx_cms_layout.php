...
class ux_tx_cms_layout extends tx_cms_layout {
    ...
    function getTable_tx_workshops($id) {
	global $TCA;
	...
	$lMarg=1;
	$showHidden = $this->tt_contentConfig['showHidden']?'':t3lib_BEfunc::BEenableFields('tx_workshops');
	...
	if (!$this->tt_contentConfig['single'])	{		// MULTIPLE column display mode, side by side:
		...
		foreach($langListArr as $lP)	{	// If NOT languageMode, then we'll only be through this once.
			...
			#foreach($cList as $key)	{
				...
				$queryParts = $this->makeQueryArray('tx_workshops', $id, $showHidden.$showLanguage);
				...
				foreach($rowArr as $row)	{
					...
					$singleElementHTML.= $this->workshops_drawHeader($row,$this->tt_contentConfig['showInfo']?15:5, $this->defLangBinding && $lP>0, TRUE);

					$isRTE = $RTE && $this->isRTEforField('tx_workshops',$row,'bodytext');
			...
			foreach($cList as $k => $key)	{
				if (!$k)	{
					$out.= '     <td><img src="clear.gif" width="'.$lMarg.'" height="1" alt="" /></td>';
				} else {
					$out.= '     <td><img src="clear.gif" width="4" height="1" alt="" /></td>
...
#debug (array($cCont,$sCont,$languageColumn));
	...
	function linkEditContent($str,$row) {
		...
		if ($this->doEdit)	{
			...
			$onClick=t3lib_BEfunc::editOnClick('&edit[tx_workshops]['.$row['uid'].']=edit',$this->backPath);
		}
		// Return link
	...
	/**
	 * ux_tx_cms_layout::tx_workshops_drawHeader()
	 *
	 * */
	 *
	 * @param	$row
	 ...
	 * @return
	 */
	function tx_workshops_drawHeader($row,$space=0,$disableMoveAndNewButtons=FALSE,$langMode=FALSE)	{
		global $TCA;

		// Load full table description:
		t3lib_div::loadTCA('tx_workshops');

		// Get record locking status:
		if ($lockInfo=t3lib_BEfunc::isRecordLocked('tx_workshops',$row['uid']))	{
		...
		$header = $this->getIcon('tx_workshops',$row).
				$lockIcon.
		...
		$out = '<tr><td class="bgColor4">'.$header.'</td></tr>';
		...
		// If show info is set...;
			...
			$info = Array();
			#$this->getProcessedValue('tx_workshops','hidden,starttime,endtime,fe_group,datetime,archivedate,title,short,image',$row,$info);
			...
			if ($this->tt_contentConfig['showCommands'] && $this->doEdit)	{
				...
				$params='&edit[tx_workshops]['.$this->tt_contentData['nextThree'][$row['uid']].']=edit';
				...
				if (!$disableMoveAndNewButtons)	{
					...
					if ($this->option_newWizard)	{
						...
						$params='&edit[tx_workshops]['.(-$row['uid']).']=new';
					...
					if ($this->tt_contentData['prev'][$row['uid']])	{
						$params='&cmd[tx_workshops]['.$row['uid'].'][move]='.$this->tt_contentData['prev'][$row['uid']];
					...
					if ($this->tt_contentData['next'][$row['uid']])	{
						$params='&cmd[tx_workshops]['.$row['uid'].'][move]='.$this->tt_contentData['next'][$row['uid']];
						$out.='<a href="'.htmlspecialchars($GLOBALS['SOBE']->doc->issueCommand($params)).'">'.
				...
				// Hide element:
				$hiddenField = $TCA['tx_workshops']['ctrl']['enablecolumns']['disabled'];
				if ($hiddenField && $TCA['tx_workshops']['columns'][$hiddenField] && (!$TCA['tx_workshops']['columns'][$hiddenField]['exclude'] || $GLOBALS['BE_USER']->check('non_exclude_fields','tt_content:'.$hiddenField)))	{
					if ($row[$hiddenField])	{
						$params='&data[tx_workshops]['.$row['uid'].']['.$hiddenField.']=0';
						$out.='<a href="'.htmlspecialchars($GLOBALS['SOBE']->doc->issueCommand($params)).'">'.
								'<img'.t3lib_iconWorks::skinImg($this->backPath,'gfx/button_unhide.gif','width="11" height="10"').' title="'.$GLOBALS['LANG']->getLL('unHide',1).'" alt="" />'.
								'</a>';
					} else {
						$params='&data[tx_workshops]['.$row['uid'].']['.$hiddenField.']=1';
						$out.='<a href="'.htmlspecialchars($GLOBALS['SOBE']->doc->issueCommand($params)).'">'.
				...
				// Delete
				$params='&cmd[tx_workshops]['.$row['uid'].'][delete]=1';
			...
			if (count($info))	{
				$out.= '<tr><td class="bgColor4-20">'.implode('<br />',$info).'</td></tr>';
			}
		#}
